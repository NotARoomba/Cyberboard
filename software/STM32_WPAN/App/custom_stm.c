...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomSensor_ServiceHdle;                    /**< SENSOR_SERVICE handle */
  uint16_t  CustomImu_DataHdle;                  /**< IMU_DATA handle */
  uint16_t  CustomBaro_DataHdle;                  /**< BARO_DATA handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeImu_Data = 28;
uint16_t SizeBaro_Data = 12;
...
#define COPY_SENSOR_SERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_IMU_DATA_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_BARO_DATA_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x02,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomImu_DataHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_IMU_DATA_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_IMU_DATA_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomBaro_DataHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_BARO_DATA_NOTIFY_DISABLED_EVT;
                Custom_STM_App_Notification(&Notification);
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_BARO_DATA_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          }  /* if (attribute_modified->Attr_Handle == (CustomContext.CustomBaro_DataHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/
        ...
        case ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE :
          ...
          if (read_req->Attribute_Handle == (CustomContext.CustomImu_DataHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
          ...
          else if (read_req->Attribute_Handle == (CustomContext.CustomBaro_DataHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
          ...
          } /* if (read_req->Attribute_Handle == (CustomContext.CustomBaro_DataHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for SENSOR_SERVICE +
   *                                2 for IMU_DATA +
   *                                2 for BARO_DATA +
   *                                1 for IMU_DATA configuration descriptor +
   *                                1 for BARO_DATA configuration descriptor +
   *                                1 for IMU_DATA broadcast property +
   *                                1 for BARO_DATA broadcast property +
   *                              = 9
   *
  ...
  COPY_SENSOR_SERVICE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomSensor_ServiceHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: SENSOR_SERVICE, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: SENSOR_SERVICE , handle = 0x%04x \n\r", CustomContext.CustomSensor_ServiceHdle);
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomSensor_ServiceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeImu_Data,
                          ...
                          &(CustomContext.CustomImu_DataHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : IMU_DATA, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : IMU_DATA , handle = 0x%04x \n\r", CustomContext.CustomImu_DataHdle);
  }
   ...
   *  BARO_DATA
   */
  COPY_BARO_DATA_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomSensor_ServiceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeBaro_Data,
                          CHAR_PROP_BROADCAST | CHAR_PROP_READ | CHAR_PROP_NOTIFY,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomBaro_DataHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : BARO_DATA, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : BARO_DATA , handle = 0x%04x \n\r", CustomContext.CustomBaro_DataHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_IMU_DATA:
      ret = aci_gatt_update_char_value(CustomContext.CustomSensor_ServiceHdle,
                                       CustomContext.CustomImu_DataHdle,
                                       0, /* charValOffset */
                                       SizeImu_Data, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value IMU_DATA command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value IMU_DATA command\n\r");
      }
    ...
    case CUSTOM_STM_BARO_DATA:
      ret = aci_gatt_update_char_value(CustomContext.CustomSensor_ServiceHdle,
                                       CustomContext.CustomBaro_DataHdle,
                                       0, /* charValOffset */
                                       SizeBaro_Data, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value BARO_DATA command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value BARO_DATA command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_IMU_DATA:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomSensor_ServiceHdle, CustomContext.CustomImu_DataHdle, SizeImu_Data, pPayload);
    ...
    case CUSTOM_STM_BARO_DATA:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomSensor_ServiceHdle, CustomContext.CustomBaro_DataHdle, SizeBaro_Data, pPayload);
